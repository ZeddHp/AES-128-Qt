...
QString AES::encrypt(const QString &plaintext) {
    ...
    EncryptBlock(block);
    QByteArray raw(reinterpret_cast<char*>(block.data()), block.size());
    return QString(raw.toBase64());
}

QString AES::decrypt(const QString &ciphertext) {
    QByteArray raw = QByteArray::fromBase64(ciphertext.toUtf8());
    std::vector<uint8_t> block(raw.begin(), raw.end());
    DecryptBlock(block);
...
uint8_t AES::mul(uint8_t a, uint8_t b) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        a <<= 1;
        if (hi_bit_set) {
            a ^= 0x1b; // Polynomial used in AES for modulo operation
        }
        b >>= 1;
    }
    return p;
...
void AES::ShiftRows(std::vector<uint8_t>& state) {
    ...
    // Each byte in a row is shifted left by the number of its row number
    for (int r = 0; r < 4; r++) {
        for (int c = 0; c < 4; c++) {
...
void AES::InvShiftRows(std::vector<uint8_t>& state) {
    ...
    // Row 0 remains unchanged
    // Each byte in a row is shifted right by the number of its row number
    for (int r = 0; r < 4; r++) {
        for (int c = 0; c < 4; c++) {
...
void AES::unpadData(std::vector<uint8_t>& data) {
    ...
    if (paddingValue > 0 && paddingValue <= 16) {  // Validate padding
